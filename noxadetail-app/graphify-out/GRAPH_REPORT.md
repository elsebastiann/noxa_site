...
1. `make_user()` - 115 edges
2. `login_as()` - 84 edges
3. `Base Layout Template` - 56 edges
4. `bogota_now()` - 30 edges
...
Cohesion: 0.12
Nodes (16): expenses_export(), liquidacion_instaladores_view(), Parking, parking_delete(), parking_list(), parking_new(), _parse_date(), Cuánto se le debe a cada instalador en el periodo, trabajo por trabajo. (+8 more)

### Community 2 - "_procesar_lead_de_meta"
Cohesion: 0.25
...
### Community 7 - "app.py"
Cohesion: 0.04
Nodes (33): api_plans_by_plate(), ensure_adjustment_base_schema(), ensure_appointment_plan_schema(), ensure_outsourcing_duration_schema(), ensure_payroll_schema(), ensure_service_sales_schema(), _fetch_twilio_media_base64(), inject_user() (+25 more)
...
### Community 10 - "route"
Cohesion: 0.07
Nodes (34): agreements_create_alias(), agreements_quick_create(), api_client_by_name(), api_public_stats_appointments_count(), expense_categories_rename(), index(), installer_toggle(), La lista de precios como matriz: una fila por servicio, una columna por tipo de… (+26 more)
...
Cohesion: 0.11
Nodes (20): abreviar_servicio(), abreviar_servicios(), api_events(), appointment_json(), appointment_money(), es_operario(), _liquidacion_instaladores(), puede_ver_precios() (+12 more)
...
Cohesion: 0.14
Nodes (12): Crea servicios base si la tabla está vacía., Crea o actualiza el precio de una celda de la matriz. Hace falta aparte de…, Gestión simple de servicios: ver y agregar nuevos., run_migrate_prices(), seed_new_services(), seed_services(), Service, service_prices_cell() (+4 more)
...
### Community 22 - "date"
Cohesion: 0.13
Nodes (14): _domingo_de_pascua(), es_festivo(), festivos_colombia(), _format_festivos_for_prompt(), _parse_fecha(), Nombre del festivo si esa fecha lo es, o None., Algoritmo de Meeus/Jones/Butcher (calendario gregoriano)., Ley Emiliani: si ya es lunes se queda; si no, se corre al lunes siguiente. (+6 more)
...
Cohesion: 0.08
Nodes (27): agreements_list(), agreements_new(), agreements_toggle(), appointments_list(), calendar_diagnosticos(), calendar_view(), logout(), notifications_list() (+19 more)
...
### Community 31 - "User"
Cohesion: 0.12
Nodes (14): api_client_names(), api_client_plates(), change_password(), _is_safe_redirect_target(), login(), True si el empleado aún está en período de prueba (primer mes desde hire_date)., Evita "open redirect": el 'next' debe ser una ruta propia (/algo), nunca una…, seed_superadmin() (+6 more)

### Community 33 - "test_festivos.py"
Cohesion: 0.09
Nodes (17): festivo_en_la_ventana(), _proximo(), proximo_domingo(), proximo_habil(), fixture, NOXA no atiende domingos ni festivos colombianos. Los festivos no se pueden…, Marca como festivo un día hábil próximo, inyectándolo en el caché. El…, La BD semilla no trae servicio de diagnóstico, así que se crea uno. Sin esto… (+9 more)
...
### Community 43 - "test_seguimiento.py"
Cohesion: 0.09
Nodes (21): _cita(), _columna(), _conv(), _limpio(), _msg(), fixture, Tablero de seguimiento: leads y clientes pendientes de contactar. Existe porque…, Cadencia del negocio: lavada premium cada 3-4 semanas. (+13 more)
...
### Community 49 - "_can_see_notifications"
Cohesion: 0.11
Nodes (18): api_notifications(), _can_see_notifications(), dashboard_gerencial(), _filtro_hace_cuanto(), notification_mark_read(), notifications_mark_all_read(), promo_image(), promotions_delete() (+10 more)

### Community 50 - "bogota_now"
Cohesion: 0.10
Nodes (25): bogota_now(), book_diagnostic_from_bot(), _diagnostic_service(), _find_active_appointment_by_plate(), _job_admin_reminder(), _job_ceramic_3weeks(), _job_ceramic_followup(), _job_client_reminder() (+17 more)

### Community 51 - "api_public_mb_book"
Cohesion: 0.13
Nodes (18): api_dia_cerrado(), api_public_mb_availability(), api_public_mb_book(), api_public_mb_price(), calculate_estimated_amount_for_appointment(), motivo_dia_cerrado(), notify_admin_mercedes_benz_booking(), Por qué está cerrado ese día, en texto para el cliente. None si se atiende. (+10 more)

### Community 53 - "_call_claude"
Cohesion: 0.29
...
Cohesion: 0.20
Nodes (12): _clean_phone_or_default(), _gestiones_activas(), _historial_ceramico(), _normalize_whatsapp_number(), {(tipo, telefono): gestión} de lo que hoy debe seguir oculto., {telefono: (fecha_ultima_visita, servicios, monto)} de citas completadas., {telefono: fecha del último cerámico o de su último mantenimiento}. Se mira el…, Arma el tablero completo. Cada persona cae en UNA sola columna. (+4 more)

### Community 56 - "make_user"
Cohesion: 0.12
Nodes (9): make_user(), Idempotencia: una conversación que YA tiene calificación no se toca, así que…, TestRutaBackfill, TestInTrial, Quedan dos capas: el allowlist global OPERARIO_ENDPOINTS lo rebota con un 302…, TestAcceso, TestPantalla, Borrarlo dejaría sin nombre la liquidación de las citas viejas. (+1 more)
...
### Community 62 - "api_estimate_price"
Cohesion: 0.15
Nodes (15): Agreement, api_estimate_price(), apply_adjustments(), apply_agreement_discount(), apply_agreement_discount_split(), appointment_already_closed(), calculate_real_price(), close_appointment() (+7 more)
...
Cohesion: 0.13
Nodes (17): _estados_entrega(), _filtro_dia_bogota(), _filtro_hora_bogota(), _filtro_sin_tildes(), Versión sin tildes de un texto, para buscar sin escribirlas., Etiqueta del separador de día en el chat: "Hoy", "Ayer" o la fecha., Los timestamps se guardan en UTC naive (datetime.utcnow). Mostrarlos tal cual…, Orden cronológico, más reciente primero — el orden por defecto de cualquier… (+9 more)

### Community 65 - "Appointment"
Cohesion: 0.24
...
Cohesion: 0.05
Nodes (38): agrupar_servicios(), api_plan_price(), AppointmentOutsourcing, categoria_de_servicio(), _citas_sin_reclasificar(), es_marketing(), _format_planes_for_prompt(), plan_toggle() (+30 more)
...
Cohesion: 0.22
Nodes (9): delete_appointment(), liberar_plan_de_cita(), Devuelve el cupo cuando la cita se cancela o se borra., Borrar una cita es irreversible y se pierde el historial del cliente, así que…, Appointments List (DataTable), Per-column Filter Row (text/select/date-range), Excel/CSV Export Buttons, Work Status Timer Controls (Iniciar/Pausar/Terminar) (+1 more)
...
### Community 78 - "generate_followup_message"
Cohesion: 0.50
...
### Community 79 - "_status_callback_url"
Cohesion: 0.67
...
- **Why does `make_user()` connect `make_user` to `make_admin`, `test_abonos_ajustes.py`, `test_archivar_conversaciones.py`, `login_as`, `test_backfill_calificacion.py`, `_cita`, `TestAgendaDeDiagnosticos`, `User`, `test_festivos.py`, `test_saldos.py`, `test_lista_precios.py`, `datetime`, `test_seguimiento.py`, `TestVistaPreviaDelPrecio`, `TestTiempoAdicional`, `conftest.py`, `test_parqueadero.py`, `.test_sin_porcentaje_valido_cae_al_del_catalogo`, `TestPanelManual`?**
  _High betweenness centrality (0.132) - this node is a cross-community bridge._
- **Why does `login_as()` connect `login_as` to `test_festivos.py`, `make_admin`, `conftest.py`, `test_abonos_ajustes.py`, `test_saldos.py`, `test_lista_precios.py`, `test_archivar_conversaciones.py`, `datetime`, `TestPanelManual`, `test_backfill_calificacion.py`, `test_parqueadero.py`, `test_seguimiento.py`, `.test_sin_porcentaje_valido_cae_al_del_catalogo`, `TestTiempoAdicional`, `make_user`, `TestVistaPreviaDelPrecio`, `_cita`, `TestAgendaDeDiagnosticos`?**
