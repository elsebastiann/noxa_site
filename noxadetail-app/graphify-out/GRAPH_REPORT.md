...
## Corpus Check
- 37 files · ~161,749 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2090 nodes · 3996 edges · 99 communities (90 shown, 9 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 79 edges (avg confidence: 0.81)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `849c4d6d`
...
7. `_borrar()` - 28 edges
8. `make_admin()` - 28 edges
9. `_cotizacion()` - 27 edges
10. `_conv()` - 26 edges
...
### Community 1 - "_build_message_history"
Cohesion: 0.17
...
### Community 4 - "_ajuste"
Cohesion: 0.08
Nodes (14): AppointmentPayment, Un abono: plata que el cliente ya entregó a cuenta del servicio. OJO — esto NO…, _abono(), _ajuste(), catalogo(), fixture, Con convenio de por medio, un 10% sobre lista y un 10% sobre subtotal son plata…, Un servicio con precio real para un tipo de vehículo, del seed. (+6 more)
...
### Community 7 - "app.py"
Cohesion: 0.03
Nodes (65): Agreement, agreements_create_alias(), agreements_quick_create(), api_plans_by_plate(), apply_agreement_discount(), apply_agreement_discount_split(), _backfill_public_tokens(), ensure_adjustment_base_schema() (+57 more)
...
### Community 13 - "bogota_now"
Cohesion: 0.07
Nodes (36): bogota_now(), _clean_phone_or_default(), _gestiones_activas(), _historial_ceramico(), _job_admin_reminder(), _job_ceramic_3weeks(), _job_ceramic_followup(), _job_client_reminder() (+28 more)
...
### Community 20 - "quality_errors_new"
Cohesion: 0.20
...
### Community 22 - "date"
Cohesion: 0.05
Nodes (54): api_dia_cerrado(), api_public_mb_availability(), api_public_mb_available_days(), api_public_mb_book(), api_public_mb_price(), _appointment_capacity_profile(), _availability_vehicle_type_id(), book_diagnostic_from_bot() (+46 more)

### Community 23 - "AppointmentAdjustment"
Cohesion: 0.40
...
Cohesion: 0.06
Nodes (32): _borrar(), _cotizacion(), El link público de una cotización: interactivo y con fecha de caducidad. El…, Vence AL FINAL del día que dice el PDF, no al empezarlo., Si el link tuviera su propio plazo, tarde o temprano diría una cosa distinta de…, El código se dicta por teléfono y se imprime; con 6 caracteres no sirve de…, Adivinar un código no puede alcanzar para ver la cotización., El cliente cambia de marca y los precios se recalculan en su navegador, sin… (+24 more)
...
Cohesion: 0.05
Nodes (46): abreviar_servicio(), abreviar_servicios(), api_estimate_price(), api_events(), apply_adjustments(), appointment_already_closed(), appointment_json(), appointment_money() (+38 more)
...
Cohesion: 0.05
Nodes (40): agrupar_servicios(), api_preguntar(), _catalogo_para_cotizar(), _catalogo_ppf(), categoria_de_servicio(), _construir_pdf_cotizacion(), _cop(), delete_service() (+32 more)
...
Cohesion: 0.06
Nodes (34): agreements_list(), agreements_new(), agreements_toggle(), appointments_list(), calendar_diagnosticos(), calendar_view(), logout(), notifications_list() (+26 more)
...
Cohesion: 0.06
Nodes (42): api_client_by_plate(), Appointment, AppointmentOperator, calculate_real_duration_minutes(), Client, ClientPlan, edit_appointment(), _guardar_tercerizacion() (+34 more)
...
Cohesion: 0.08
Nodes (26): _analytics_data(), _costo_de_la_llamada(), _ejecutar_consulta_lectura(), es_cita_de_diagnostico(), _esquema_para_preguntas(), _kpis_diagnosticos(), _kpis_rentabilidad(), _meses_del_periodo() (+18 more)

### Community 71 - "analytics_dashboard"
Cohesion: 0.09
Nodes (24): analytics_dashboard(), analytics_detalle(), _kpis_clientes(), _kpis_embudo(), _kpis_operacion(), _rango(), _rango_utc(), Límites para campos guardados en hora LOCAL de Bogotá, como… (+16 more)

### Community 72 - "limit"
Cohesion: 0.11
Nodes (14): api_client_names(), api_client_plates(), api_notifications(), quote_public(), quote_public_pdf(), quote_public_seleccion(), QuoteVersion, Lo que el cliente armó por su cuenta desde el link. NO toca la cotización… (+6 more)

### Community 73 - "make_user"
Cohesion: 0.09
Nodes (11): make_user(), Idempotencia: una conversación que YA tiene calificación no se toca, así que…, TestRutaBackfill, TestInTrial, El backend no debe rechazarlas: son un SQL válido, y la tabla las muestra bien.…, Quedan dos capas: el allowlist global OPERARIO_ENDPOINTS lo rebota con un 302…, TestAcceso, El desglose que se ve al agendar sale del servidor, con la misma fórmula que… (+3 more)
...
Cohesion: 0.11
Nodes (21): Expense, expenses_edit(), expenses_export(), expenses_list(), expenses_new(), expenses_toggle_void(), get_existing_vendors(), _parse_date() (+13 more)
...
Cohesion: 0.10
Nodes (13): catalogo(), fixture, Cotizaciones: código único, precios congelados y PDF reimprimible. Lo delicado…, El punto entero del diseño., Un servicio con dos precios distintos según el vehículo — que es justamente lo…, Servicios que no están en sistema: un trabajo especial, un insumo puntual. Se…, Salían dos líneas diciendo lo mismo con otras palabras, y un pie que se repite…, Como el precio: si mañana cambia, lo ya entregado tiene que seguir diciendo lo… (+5 more)
...
### Community 98 - "Quote"
Cohesion: 0.09
Nodes (11): Quote, [(marca, garantía), ...] como estaban al emitir la cotización., {cobertura: la cobertura total que ya la incluye}. La regla vive acá y no en…, Cuánto vale una selección parcial. Se calcula ACÁ, con los precios que están…, {marca: total}. No suma lo que la marca no ofrece ni lo que Full Car ya cubre., {marca: [coberturas que esa marca no ofrece]}. Hay que decirlo en el documento.…, {marca: total final} = servicios + PPF de esa marca, ya con descuento. El…, Una cotización que se le entrega al cliente y se puede volver a consultar. Todo… (+3 more)
...
Cohesion: 0.12
Nodes (15): _guardar_media_entrante(), _linea_perfil(), MessageMedia, _nombre_perfil_utilizable(), notify_admin_conversation_error(), El nombre de perfil de WhatsApp lo escribe el cliente y muchas veces no es un…, La línea de nombre que se le pasa al modelo, ya filtrada., Archivo (normalmente una foto) que llegó adjunto a un mensaje. Se guarda una… (+7 more)
...
- **Why does `make_user()` connect `make_user` to `make_admin`, `test_archivar_conversaciones.py`, `test_backfill_calificacion.py`, `User`, `_borrar`, `_cita`, `test_saldos.py`, `login_as`, `test_servicios_ui.py`, `_conv`, `datetime`, `TestFormulario`, `TestTiempoAdicional`, `test_preguntar_datos.py`, `test_lista_precios.py`, `TestAgendaDeDiagnosticos`, `._login_admin`, `test_festivos.py`, `._preguntar`, `_claude_responde`, `_borrar`, `test_parqueadero.py`, `test_colores_agenda.py`, `test_cotizaciones.py`, `._login`, `TestAgrupacion`?**
  _High betweenness centrality (0.272) - this node is a cross-community bridge._
- **Why does `login_as()` connect `login_as` to `make_admin`, `test_archivar_conversaciones.py`, `test_backfill_calificacion.py`, `_cita`, `test_saldos.py`, `test_servicios_ui.py`, `_conv`, `datetime`, `TestFormulario`, `TestTiempoAdicional`, `test_preguntar_datos.py`, `test_lista_precios.py`, `TestAgendaDeDiagnosticos`, `make_user`, `test_festivos.py`, `._preguntar`, `_claude_responde`, `test_parqueadero.py`, `test_colores_agenda.py`, `TestAgrupacion`?**
  _High betweenness centrality (0.109) - this node is a cross-community bridge._
- **Why does `User` connect `User` to `make_user`, `Conversation`, `app.py`?**
