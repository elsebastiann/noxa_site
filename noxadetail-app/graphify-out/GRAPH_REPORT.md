...
## Corpus Check
- 19 files · ~105,543 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1097 nodes · 2211 edges · 67 communities (62 shown, 5 thin omitted)
- Extraction: 97% EXTRACTED · 3% INFERRED · 0% AMBIGUOUS · INFERRED: 71 edges (avg confidence: 0.81)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `35a78ae6`
...
### Community 7 - "app.py"
Cohesion: 0.04
Nodes (35): ensure_adjustment_base_schema(), ensure_appointment_plan_schema(), ensure_payroll_schema(), ensure_service_sales_schema(), _estados_entrega(), _fetch_twilio_media_base64(), inject_user(), MaintenancePlan (+27 more)
...
Cohesion: 0.20
Nodes (11): _build_message_history(), _call_claude(), _clasificar_conversacion_historica(), _get_claude_client(), _match_valor_cerrado(), Historial de la conversación en formato Claude. Claude exige alternancia…, Llama a Claude con la base de conocimiento de NOXA + contexto puntual, y parte…, Backfill: clasifica una conversación existente (estado/servicios/carro/marca/… (+3 more)
...
Cohesion: 0.11
Nodes (16): dashboard_gerencial(), Parking, parking_delete(), parking_list(), parking_new(), _parse_date(), Los pocos números que un dueño necesita para saber si el negocio va bien. Cada…, Listado de ingresos (ventas de servicios) con filtros básicos. (+8 more)
...
Cohesion: 0.08
Nodes (22): payroll_delete(), payroll_detail(), payroll_entry_update(), payroll_list(), payroll_new(), payroll_pay(), payroll_vale_new(), PayrollPeriod (+14 more)
...
Cohesion: 0.14
Nodes (15): bogota_now(), _job_ceramic_3weeks(), _job_ceramic_followup(), _job_client_reminder(), _job_reengagement_followup(), notify_admin_gestion_cliente(), plans_list(), Activa y dentro de fechas. Las fechas vacías significan "sin límite". (+7 more)
...
### Community 40 - "delete_appointment"
Cohesion: 0.50
...
Cohesion: 0.22
Nodes (9): api_client_by_plate(), Client, normalize_plate(), plan_sell(), Normaliza placa: trim, sin espacios internos, mayúsculas., Crea o actualiza el cliente por placa., Devuelve datos de cliente por placa. Uso: /api/clients/by-plate?plate=ABC123, Vende un plan y registra el ingreso. La plata entra hoy, completa: es prepago.… (+1 more)
...
Cohesion: 0.16
Nodes (15): api_dia_cerrado(), _appointment_capacity_profile(), book_diagnostic_from_bot(), _day_business_end(), _find_active_appointment_by_plate(), get_available_slots(), motivo_dia_cerrado(), Por qué está cerrado ese día, en texto para el cliente. None si se atiende. (+7 more)
...
Cohesion: 0.18
Nodes (12): edit_appointment(), Guardia de servidor para las citas creadas a mano. El aviso en pantalla se…, _requiere_confirmar_dia_cerrado(), Appointment Form (Shared Partial), Multiple Discount/Surcharge Rows, Agreement Dropdown with Inline Quick-Create, Client Autocomplete by Plate/Name, Multiple Partial Payments (Abonos) Rows (+4 more)
...
Cohesion: 0.18
Nodes (11): analytics_detalle(), _diagnostic_service(), _job_post_service_followup(), _kpis_diagnosticos(), _nombre_servicio_diagnostico(), Toda cita agendada cuenta como servicio prestado — así opera el negocio. El…, El diagnóstico es la puerta de entrada del negocio: es gratis y solo se…, Qué hay detrás de un punto de una gráfica. Un número agregado sin poder abrirlo… (+3 more)

### Community 70 - "new_appointment"
Cohesion: 0.15
Nodes (14): Appointment, AppointmentOperator, calculate_estimated_amount_for_appointment(), calculate_real_duration_minutes(), _int_o_cero(), new_appointment(), Calcula duración total real usando ServicePrice. Estrategia: - Suma todas las…, Lo que vale el servicio: precio de lista, menos convenio, más/menos los… (+6 more)
...
- **Why does `make_user()` connect `make_user` to `login_as`, `test_festivos.py`, `TestPanelManual`, `test_abonos_ajustes.py`, `make_admin`, `test_archivar_conversaciones.py`, `test_backfill_calificacion.py`, `User`, `datetime`, `TestRegresionProduccion`, `test_parqueadero.py`, `conftest.py`?**
  _High betweenness centrality (0.109) - this node is a cross-community bridge._
- **Why does `login_as()` connect `login_as` to `test_festivos.py`, `TestPanelManual`, `make_admin`, `test_abonos_ajustes.py`, `test_archivar_conversaciones.py`, `make_user`, `test_backfill_calificacion.py`, `datetime`, `TestRegresionProduccion`, `test_parqueadero.py`, `conftest.py`?**
