...
## Corpus Check
- 53 files · ~186,770 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2649 nodes · 4978 edges · 187 communities (148 shown, 39 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 107 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `455d7a02`
...
Cohesion: 0.08
Nodes (12): make_user(), TestInTrial, Preguntarle a la data da acceso a toda la plata de una forma que ningún tablero…, Un admin pasa el allowlist global, así que llega hasta la ruta y es MI candado…, Queda fuera antes de llegar a la ruta: dos capas, y la de afuera actúa primero…, TestQuienPuedeEntrar, Los saldos son información de la cuenta, no de la operación diaria., TestPaginaEstado (+4 more)

### Community 20 - "login_as"
Cohesion: 0.11
Nodes (10): login_as(), El formulario manda listas paralelas; acá se prueba el parseo., El default acordado con la operación: si nadie elige, es sobre lista., TestFormulario, TestLineasDelEvento, TestApiDiaCerrado, Un precio con duración 0 hace que la cita no ocupe tiempo en el calendario, y…, Es la razón de que exista el endpoint aparte: /update exige un ServicePrice que… (+2 more)
...
### Community 29 - "_cita"
Cohesion: 0.31
Nodes (4): _cita(), Un PPF a medida no tiene fila en ServicePrice: sin esto la cita valdría 0 y el…, TestLiquidacion, TestTrabajosAMedida

### Community 30 - "route"
Cohesion: 0.09
Nodes (26): api_client_by_name(), api_client_by_phone(), api_public_stats_appointments_count(), expense_categories_rename(), index(), installer_toggle(), La lista de precios como matriz: una fila por servicio, una columna por tipo de…, Desactivar en vez de borrar: las citas viejas siguen apuntando a él y borrarlo… (+18 more)
...
Cohesion: 0.06
Nodes (38): analytics_dashboard(), _analytics_data(), analytics_detalle(), _can_see_notifications(), dashboard_gerencial(), _kpis_clientes(), _kpis_embudo(), _kpis_operacion() (+30 more)
...
Cohesion: 0.05
Nodes (46): agrupar_servicios(), api_preguntar(), _catalogo_para_cotizar(), _catalogo_ppf(), categoria_de_servicio(), delete_service(), dia_bogota(), garantia_texto() (+38 more)
...
Cohesion: 0.05
Nodes (34): api_public_web_lead(), _build_web_lead_opening_text(), Conversation, _is_safe_redirect_target(), _log_outbound(), login(), Message, notify_admin_new_web_lead() (+26 more)
...
Cohesion: 0.17
Nodes (15): api_public_mb_availability(), api_public_mb_available_days(), api_public_mb_book(), bogota_today(), calculate_estimated_amount_for_appointment(), get_available_days(), _job_client_reminder(), Corre diariamente a las 7 PM (Bogotá). Notifica a clientes con cita mañana. (+7 more)
...
### Community 60 - "date"
Cohesion: 0.13
Nodes (21): api_dia_cerrado(), _domingo_de_pascua(), es_festivo(), festivos_colombia(), _format_festivos_for_prompt(), _kpis_diagnosticos(), motivo_dia_cerrado(), Festivos que caen dentro de la ventana de agendamiento. El bloque de… (+13 more)
...
### Community 62 - "book_diagnostic_from_bot"
Cohesion: 0.13
...
Cohesion: 0.09
Nodes (24): _dia_bogota_iso(), _estados_entrega(), _filtro_hace_cuanto(), _filtro_hora_bogota(), _filtro_sin_tildes(), _quien(), El día en Bogotá, en ISO, para el filtro de fechas de la bandeja. Va en ISO y…, Orden cronológico, más reciente primero — el orden por defecto de cualquier… (+16 more)

### Community 73 - "send_whatsapp"
Cohesion: 0.05
Nodes (46): _clasificar_conversacion_historica(), _compute_priority(), _generate_and_send_reply(), _guardar_media_entrante(), is_first_client_turn(), _job_admin_reminder(), _looks_like_welcome_menu(), _match_valor_cerrado() (+38 more)
...
### Community 79 - "get_claude_reply"
Cohesion: 0.14
Nodes (14): _format_availability_for_prompt(), _format_planes_for_prompt(), _format_prices_for_prompt(), _format_promotions_for_prompt(), get_claude_reply(), _media_base64(), Lee un adjunto ya guardado y lo devuelve en base64 para mandárselo a Claude., Tabla de precios real, leída de `service_prices` en cada turno. El catálogo… (+6 more)
...
Cohesion: 0.18
Nodes (11): api_plan_price(), api_public_mb_price(), calculate_real_price(), precio_sugerido_plan(), Precio sugerido para el combo plan × tipo de vehículo, para el formulario., Servicio activo por nombre exacto, sin distinguir mayúsculas ni espacios., Cuánto vale el plan para ese tipo de vehículo. Es la suma de los servicios que…, Calcula el precio base real usando ServicePrice. Estrategia: - Suma los precios… (+3 more)

### Community 89 - "TestEliminar"
Cohesion: 0.18
...
### Community 94 - "test_tercerizacion.py"
Cohesion: 0.22
Nodes (8): catalogo(), fixture, Servicios tercerizados: polarizado, PPF y wrap. Los hace un instalador externo…, Aplicar el % al total de la cita le regalaría al instalador un pedazo del…, Un servicio tercerizado con precio de lista, uno a medida y uno propio., Las citas de prueba caen todas en el mismo día, así que si una sobrevive al…, _sin_citas_de_otros_tests(), TestRepartoPorServicio

### Community 95 - "test_cotizaciones.py"
Cohesion: 0.08
Nodes (15): catalogo(), fixture, Cotizaciones: código único, precios congelados y PDF reimprimible. Lo delicado…, Como el precio: si mañana cambia, lo ya entregado tiene que seguir diciendo lo…, En el PDF los precios de lo absorbido SÍ se ven, en gris, pero no suman. Sirven…, Se rendiriza sin reventar con filas absorbidas de las dos zonas., Con dos coberturas totales, cada fila tiene que nombrar la suya., Servicios que no están en sistema: un trabajo especial, un insumo puntual. Se… (+7 more)
...
### Community 98 - "Quote"
Cohesion: 0.05
Nodes (24): absorbidas_en(), _construir_pdf_cotizacion(), _cop(), _ppf_no_cubre_en(), ppf_totales_de(), Quote, QuotePpfItem, Solo los servicios. El PPF no entra aquí porque no tiene UN precio: tiene uno… (+16 more)

### Community 99 - "Base Layout Template"
Cohesion: 0.10
Nodes (20): calendar_diagnosticos(), change_password(), logout(), notifications_list(), payment_methods_list(), payment_methods_toggle(), quality_errors_list(), Historial completo, para cuando la campanita se queda corta. (+12 more)
...
### Community 104 - "TestReclasificacionHistorica"
Cohesion: 0.25
Nodes (4): Los polarizados y PPF ya registrados cuentan el ingreso completo como de Noxa.…, Aplicarla dos veces le duplicaría el costo al instalador., El reparto no siempre fue el mismo: aplicar 65% a ciegas cambiaría un error por…, TestReclasificacionHistorica
...
Cohesion: 0.12
Nodes (15): AppointmentOutsourcing, _citas_sin_reclasificar(), es_marketing(), _liquidacion_instaladores(), liquidacion_instaladores_view(), plan_toggle(), puede_ver_finanzas(), Desactiva un plan vendido (venta anulada, cliente que se fue). (+7 more)

### Community 109 - "bogota_now"
Cohesion: 0.10
Nodes (18): bogota_now(), _filtro_dia_bogota(), _job_ceramic_3weeks(), _job_ceramic_followup(), _job_reengagement_followup(), notify_admin_gestion_cliente(), plans_list(), Le avisa a Diana que hay un cliente que ella tiene que contactar. Estos… (+10 more)
...
### Community 113 - "limit"
Cohesion: 0.13
Nodes (15): api_client_names(), api_client_plates(), api_notifications(), _guardar_version_cliente(), quote_public(), quote_public_pdf(), quote_public_seleccion(), quotes_list() (+7 more)

### Community 114 - "TestInstaladoPorNoxa"
Cohesion: 0.25
Nodes (4): Algunos PPF los instala el propio equipo: no hay comisión y el ingreso queda…, En una liquidación solo estorba: no hay nada que pagarle a nadie., Sin instalador y sin comisión = lo hizo el equipo. Sin instalador pero con…, TestInstaladoPorNoxa
...
### Community 117 - "TestPromptExigeDosColumnas"
Cohesion: 0.29
...
### Community 125 - "appointment_money"
Cohesion: 0.22
Nodes (9): appointment_money(), appointments_list(), Todo el desglose de plata de una cita, en un solo lugar. La distinción que…, Lista simple en tabla de las próximas citas., Appointments List (DataTable), Per-column Filter Row (text/select/date-range), Excel/CSV Export Buttons, Work Status Timer Controls (Iniciar/Pausar/Terminar) (+1 more)
...
### Community 129 - "TestRepartoBasico"
Cohesion: 0.33
Nodes (3): La gran mayoría de citas no se reparten: no pueden verse afectadas., Tercerizar no cambia lo que se le cobra: cambia a quién le queda., TestRepartoBasico
...
Cohesion: 0.33
Nodes (5): apply_adjustments(), appointment_already_closed(), close_appointment(), Aplica una lista de descuentos/recargos sobre el subtotal. Cada línea en…, ServiceSale
...
### Community 140 - "_claude_responde"
Cohesion: 0.40
...
### Community 141 - "TestDescuentos"
Cohesion: 0.40
Nodes (3): Si se descuenta, el instalador no puede llevarse el 65% de una plata que nunca…, El descuento es de la cita, no de un servicio: se prorratea, así que el…, TestDescuentos

### Community 143 - "TestAnalitica"
Cohesion: 0.40
Nodes (3): El problema original: el tablero mostraba el polarizado completo como ingreso…, Si el margen saliera del facturado, un mes lleno de polarizados se vería…, TestAnalitica
...
- **Why does `make_user()` connect `make_user` to `make_admin`, `test_abonos_ajustes.py`, `TestPantallas`, `test_archivar_conversaciones.py`, `test_backfill_calificacion.py`, `_claude_responde`, `TestPanelManual`, `test_editar_conserva.py`, `_borrar`, `login_as`, `test_ajuste_precios.py`, `_borrar`, `.admin`, `test_marcas_ppf.py`, `._login_admin`, `test_saldos.py`, `test_filtro_fechas_whatsapp.py`, `TestVistaPreviaDelPrecio`, `test_servicios_ui.py`, `_conv`, `Conversation`, `TestLaPantallaDePrecios`, `._login_admin`, `test_festivos.py`, `test_parqueadero.py`, `datetime`, `test_zona_horaria.py`, `test_duplicar_cotizacion.py`, `TestTiempoAdicional`, `test_cotizacion_autorrelleno.py`, `test_preguntar_datos.py`, `test_aviso_referencia.py`, `TestAgendaDeDiagnosticos`, `test_nav_movil.py`, `test_cotizacion_publica.py`, `._preguntar`, `TestFullCarAbsorbeLoExterior`, `_borrar`, `TestEliminar`, `test_lista_precios.py`, `test_tercerizacion.py`, `test_cotizaciones.py`, `._login`, `_borrar`, `conftest.py`, `TestReclasificacionHistorica`, `test_colores_agenda.py`, `TestInstaladoPorNoxa`, `TestPromptExigeDosColumnas`?**
  _High betweenness centrality (0.347) - this node is a cross-community bridge._
- **Why does `login_as()` connect `login_as` to `make_admin`, `test_abonos_ajustes.py`, `TestPantallas`, `test_archivar_conversaciones.py`, `test_backfill_calificacion.py`, `_claude_responde`, `TestPanelManual`, `make_user`, `.admin`, `test_saldos.py`, `test_filtro_fechas_whatsapp.py`, `TestVistaPreviaDelPrecio`, `test_servicios_ui.py`, `_conv`, `test_festivos.py`, `test_parqueadero.py`, `datetime`, `TestTiempoAdicional`, `test_preguntar_datos.py`, `TestAgendaDeDiagnosticos`, `._preguntar`, `test_lista_precios.py`, `test_tercerizacion.py`, `conftest.py`, `TestReclasificacionHistorica`, `test_colores_agenda.py`, `TestInstaladoPorNoxa`, `TestPromptExigeDosColumnas`?**
  _High betweenness centrality (0.072) - this node is a cross-community bridge._
- **Why does `User` connect `Conversation` to `make_user`, `app.py`?**
