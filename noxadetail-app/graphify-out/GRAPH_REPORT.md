...
## Corpus Check
- 46 files · ~178,230 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2426 nodes · 4610 edges · 117 communities (108 shown, 9 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 86 edges (avg confidence: 0.81)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `4b9b0f77`
...
### Community 7 - "app.py"
Cohesion: 0.03
Nodes (63): Agreement, agreements_create_alias(), agreements_new(), agreements_quick_create(), api_plans_by_plate(), AppMigration, _backfill_public_tokens(), ensure_adjustment_base_schema() (+55 more)
...
Cohesion: 0.08
Nodes (21): color_hex_valido(), color_texto_legible(), Normaliza un color a #RRGGBB, o None si no lo es. El valor viaja desde un…, Negro o blanco, el que contraste con el fondo. Es el valor por defecto cuando…, Crea servicios base si la tabla está vacía., Crea o actualiza el precio de una celda de la matriz. Hace falta aparte de…, Gestión simple de servicios: ver y agregar nuevos., Color del cajón de la cita en la agenda. Se valida el hex acá y no solo en el… (+13 more)

### Community 19 - "make_user"
Cohesion: 0.07
Nodes (21): login_as(), make_user(), El formulario manda listas paralelas; acá se prueba el parseo., El default acordado con la operación: si nadie elige, es sobre lista., TestFormulario, TestApiDiaCerrado, Un precio con duración 0 hace que la cita no ocupe tiempo en el calendario, y…, Es la razón de que exista el endpoint aparte: /update exige un ServicePrice que… (+13 more)

### Community 20 - "send_whatsapp"
Cohesion: 0.04
Nodes (57): api_public_web_lead(), _build_web_lead_opening_text(), Conversation, _generate_and_send_reply(), _guardar_media_entrante(), _job_check_saldos(), _log_outbound(), _looks_like_welcome_menu() (+49 more)
...
Cohesion: 0.07
Nodes (30): analytics_detalle(), dashboard_gerencial(), Expense, expenses_edit(), expenses_export(), expenses_list(), expenses_new(), expenses_toggle_void() (+22 more)
...
Cohesion: 0.06
Nodes (38): agreements_list(), agreements_toggle(), appointments_list(), calendar_diagnosticos(), calendar_view(), delete_appointment(), logout(), parking_delete() (+30 more)
...
### Community 33 - "api_events"
Cohesion: 0.06
Nodes (31): abreviar_servicio(), abreviar_servicios(), api_events(), api_preguntar(), _costo_de_la_llamada(), _ejecutar_consulta_lectura(), es_cita_de_diagnostico(), _esquema_para_preguntas() (+23 more)
...
Cohesion: 0.08
Nodes (28): appointment_json(), _construir_pdf_cotizacion(), _cop(), delete_service(), es_operario(), garantia_texto(), _ppf_no_cubre_en(), ppf_prices_list() (+20 more)

### Community 40 - "appointment_money"
Cohesion: 0.11
Nodes (23): api_estimate_price(), apply_adjustments(), apply_agreement_discount(), apply_agreement_discount_split(), appointment_already_closed(), appointment_money(), calculate_estimated_amount_for_appointment(), calculate_real_price() (+15 more)
...
Cohesion: 0.06
Nodes (34): _agendar(), conv(), _cuantas(), _datos(), placa(), fixture, Repetir [AGENDAR:] con los mismos datos no es un error: la cita ya está. Caso…, El tercer valor es la cita que ESA llamada creó. En la repetición no creó… (+26 more)
...
### Community 55 - "_leer_formulario_de_cotizacion"
Cohesion: 0.17
...
### Community 58 - "limit"
Cohesion: 0.14
Nodes (14): api_client_names(), api_client_plates(), _guardar_version_cliente(), quote_public(), quote_public_pdf(), quote_public_seleccion(), La cotización interactiva que ve el cliente. Sin login. El cliente marca y…, Guarda lo que el cliente armó, como una versión aparte de la original. La… (+6 more)

### Community 59 - "date"
Cohesion: 0.06
Nodes (47): api_dia_cerrado(), api_public_mb_availability(), api_public_mb_available_days(), api_public_mb_book(), api_public_mb_price(), _appointment_capacity_profile(), _availability_vehicle_type_id(), _day_business_end() (+39 more)

### Community 60 - "_filtro_dia_bogota"
Cohesion: 0.22
...
### Community 62 - "quote_new"
Cohesion: 0.17
Nodes (15): agrupar_servicios(), _catalogo_para_cotizar(), _catalogo_ppf(), categoria_de_servicio(), _nuevo_codigo_cotizacion(), _partes_ppf(), ppf_marcas_activas(), quote_edit() (+7 more)

### Community 63 - "edit_appointment"
Cohesion: 0.06
Nodes (44): api_client_by_plate(), Appointment, AppointmentOperator, calculate_real_duration_minutes(), Client, ClientPlan, edit_appointment(), _guardar_tercerizacion() (+36 more)

### Community 64 - "ppf_totales_de"
Cohesion: 0.25
...
### Community 79 - "TestPromptExigeDosColumnas"
Cohesion: 0.29
...
Cohesion: 0.05
Nodes (48): bogota_now(), book_diagnostic_from_bot(), _clean_phone_or_default(), _diagnostic_service(), _find_active_appointment_by_plate(), _gestiones_activas(), _historial_ceramico(), _job_admin_reminder() (+40 more)
...
Cohesion: 0.33
Nodes (4): PpfPart, Una parte del carro que se puede forrar. Es la unidad mínima y NO tiene precio:…, Crea las partes que falten, sin tocar las que ya están., seed_ppf_parts()
...
### Community 109 - "promotions_list"
Cohesion: 0.12
Nodes (14): _parse_fecha(), Promotion, promotions_list(), _public_base_url(), Valida la firma de Twilio contra la URL EXACTA que nosotros le dimos como…, Guarda la imagen de apoyo y devuelve el nombre con el que quedó. El nombre…, Promociones que el equipo monta a mano y Mariana usa para cerrar. El texto va…, Activa y dentro de fechas. Las fechas vacías significan "sin límite". (+6 more)
...
### Community 138 - "_can_see_notifications"
Cohesion: 0.12
Nodes (16): api_notifications(), _can_see_notifications(), notification_mark_read(), notifications_list(), notifications_mark_all_read(), promo_image(), promotions_delete(), promotions_toggle() (+8 more)
...
- **Why does `make_user()` connect `make_user` to `test_lista_precios.py`, `make_admin`, `test_abonos_ajustes.py`, `test_archivar_conversaciones.py`, `test_backfill_calificacion.py`, `test_editar_conserva.py`, `foto`, `_borrar`, `_cita`, `test_marcas_ppf.py`, `._login_admin`, `test_saldos.py`, `test_filtro_fechas_whatsapp.py`, `test_servicios_ui.py`, `test_festivos.py`, `_conv`, `TestLaPantallaDePrecios`, `precio`, `User`, `datetime`, `TestTiempoAdicional`, `test_preguntar_datos.py`, `TestPanelManual`, `TestAgendaDeDiagnosticos`, `test_nav_movil.py`, `._preguntar`, `TestPromptExigeDosColumnas`, `TestFullCarAbsorbeLoExterior`, `_borrar`, `TestSeCreaSolo`, `._login_admin`, `test_parqueadero.py`, `test_cotizaciones.py`, `._login`, `.test_sin_porcentaje_valido_cae_al_del_catalogo`, `_claude_responde`, `test_cotizacion_publica.py`, `TestVistaPreviaDelPrecio`, `test_colores_agenda.py`?**
  _High betweenness centrality (0.330) - this node is a cross-community bridge._
- **Why does `login_as()` connect `make_user` to `test_lista_precios.py`, `make_admin`, `test_abonos_ajustes.py`, `test_archivar_conversaciones.py`, `test_backfill_calificacion.py`, `_cita`, `test_saldos.py`, `test_filtro_fechas_whatsapp.py`, `test_servicios_ui.py`, `test_festivos.py`, `_conv`, `datetime`, `TestTiempoAdicional`, `test_preguntar_datos.py`, `TestPanelManual`, `TestAgendaDeDiagnosticos`, `._preguntar`, `TestPromptExigeDosColumnas`, `test_parqueadero.py`, `.test_sin_porcentaje_valido_cae_al_del_catalogo`, `_claude_responde`, `TestVistaPreviaDelPrecio`, `test_colores_agenda.py`?**
