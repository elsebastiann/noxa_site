...
## Corpus Check
- 57 files · ~199,565 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2903 nodes · 5408 edges · 143 communities (136 shown, 7 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 107 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f9010948`
...
### Community 7 - "app.py"
Cohesion: 0.02
Nodes (86): api_preguntar(), _backfill_public_tokens(), _construir_pdf_cotizacion(), _cop(), delete_service(), ensure_adjustment_base_schema(), ensure_appointment_plan_schema(), ensure_outsourcing_duration_schema() (+78 more)
...
Cohesion: 0.06
Nodes (23): login_as(), make_user(), El formulario manda listas paralelas; acá se prueba el parseo., El default acordado con la operación: si nadie elige, es sobre lista., TestFormulario, TestLineasDelEvento, TestApiDiaCerrado, Un precio con duración 0 hace que la cita no ocupe tiempo en el calendario, y… (+15 more)

### Community 20 - "._login_admin"
Cohesion: 0.12
Nodes (10): En el PDF los precios de lo absorbido SÍ se ven, en gris, pero no suman. Sirven…, Se rendiriza sin reventar con filas absorbidas de las dos zonas., Con dos coberturas totales, cada fila tiene que nombrar la suya., Editar una cotización ya emitida conservando su código., Es el identificador que el cliente ya tiene; cambiarlo lo dejaría buscando una…, Si la vigencia se contara desde hoy, abrir y guardar una cotización vencida la…, Refrescarla contra la tabla cambiaría en silencio una cifra que el cliente ya…, Sin el cascade quedarían filas huérfanas apuntando a una cotización que ya no… (+2 more)
...
Cohesion: 0.04
Nodes (69): appointments_list(), calendar_diagnosticos(), calendar_view(), dashboard_gerencial(), delete_appointment(), Expense, expense_categories_delete(), expense_categories_list() (+61 more)
...
Cohesion: 0.06
Nodes (37): api_plan_price(), appointment_json(), AppointmentOutsourcing, _citas_sin_reclasificar(), es_marketing(), es_operario(), plan_toggle(), precio_sugerido_plan() (+29 more)
...
Cohesion: 0.11
Nodes (19): Agreement, agreements_create_alias(), agreements_list(), agreements_new(), agreements_quick_create(), agreements_toggle(), apply_agreement_discount(), apply_agreement_discount_split() (+11 more)
...
Cohesion: 0.14
Nodes (14): _format_planes_for_prompt(), _format_prices_for_prompt(), _format_promotions_for_prompt(), get_claude_reply(), is_first_client_turn(), _media_base64(), _phone_for_display(), Lee un adjunto ya guardado y lo devuelve en base64 para mandárselo a Claude. (+6 more)
...
Cohesion: 0.10
Nodes (12): _clean_db(), client(), fixture, Cada test arranca con las tablas de nómina/usuarios vacías, y corre dentro de…, _session_setup(), fixture, Lo que va dentro del cajón de una cita en la agenda. El recorte por alto lo…, Al abrirle la agenda a marketing dejó de existir el rebote que lo mandaba al… (+4 more)
...
Cohesion: 0.29
Nodes (5): _nuevo_codigo_solicitud(), price_request_new(), PriceRequestItem, Mismo alfabeto sin letras confundibles que los códigos de cotización, con otro…, Una parte o grupo sobre el que se pide precio, y lo que el instalador respondió…
...
Cohesion: 0.12
Nodes (12): _borrar(), _cotizacion(), 500000 sobre una cotización de 200000: sin tope, el PDF que se le entrega al…, El punto entero del diseño., Sin teléfono, sin placa, sin vehículo, sin descuento y sin notas., Crea una cotización directa en BD y devuelve su código., Borrar una cotización pide la MISMA palabra clave que borrar una cita. Una sola…, Si fueran dos palabras distintas, rotar una dejaría la otra vieja. (+4 more)
...
### Community 104 - "date"
Cohesion: 0.03
Nodes (89): api_dia_cerrado(), api_public_mb_availability(), api_public_mb_available_days(), api_public_mb_book(), api_public_mb_price(), _appointment_capacity_profile(), _availability_vehicle_type_id(), bogota_now() (+81 more)
...
### Community 113 - "route"
Cohesion: 0.03
Nodes (81): api_client_by_name(), api_client_by_phone(), api_client_names(), api_client_plates(), api_notifications(), api_price_requests_respondidas(), api_public_stats_appointments_count(), _can_see_notifications() (+73 more)
...
### Community 121 - "TestPromptExigeDosColumnas"
Cohesion: 0.29
...
Cohesion: 0.06
Nodes (37): _comparacion_serverless(), _costo_de_la_llamada(), _costo_railway(), _diagnostico_anthropic(), _ejecutar_consulta_lectura(), _esquema_para_preguntas(), estado_servicios(), _fecha_iso() (+29 more)
...
### Community 131 - "_format_availability_for_prompt"
Cohesion: 0.50
...
Cohesion: 0.12
Nodes (10): _base_publica(), _generar_miniatura(), _guardar_foto_vehiculo(), _nombre_miniatura(), PriceRequest, Guarda la foto y devuelve el nombre con el que quedó, o None. Nombre aleatorio:…, Versión liviana de la foto, para la vista previa de WhatsApp. WhatsApp descarga…, De dónde cuelgan los links que salen de la app hacia afuera. (+2 more)
...
Cohesion: 0.04
Nodes (65): api_client_by_plate(), api_estimate_price(), api_plans_by_plate(), apply_adjustments(), Appointment, appointment_already_closed(), appointment_money(), AppointmentOperator (+57 more)

### Community 154 - "TestVistaPreviaDelPrecio"
Cohesion: 0.20
Nodes (6): Un POST sin JS llegaría con el pct vacío. Sin este respaldo quedaría en 0 y el…, El desglose que se ve al agendar sale del servidor, con la misma fórmula que…, Sin sumar el valor cotizado, el PPF a medida mostraría $0 y el usuario creería…, Es la razón de que el cálculo esté compartido: si divergen, el número que se ve…, TestGuardadoDesdeElFormulario, TestVistaPreviaDelPrecio
...
Cohesion: 0.06
Nodes (29): change_password(), payroll_delete(), payroll_detail(), payroll_entry_update(), payroll_list(), payroll_new(), payroll_pay(), payroll_vale_new() (+21 more)
...
- **Why does `make_user()` connect `make_user` to `_claude_responde`, `make_admin`, `test_abonos_ajustes.py`, `.admin`, `test_archivar_conversaciones.py`, `test_backfill_calificacion.py`, `test_editar_conserva.py`, `_borrar`, `foto`, `._login_admin`, `_borrar`, `TestVistaPreviaDelPrecio`, `_lecturas`, `test_saldos.py`, `TestSeCreaSolo`, `test_marcas_ppf.py`, `test_marcas_sin_precio.py`, `test_lista_precios.py`, `test_filtro_fechas_whatsapp.py`, `test_festivos.py`, `test_servicios_ui.py`, `_conv`, `TestAgendaDeDiagnosticos`, `TestLaPantallaDePrecios`, `precio`, `test_descuento_en_pdf.py`, `datetime`, `test_zona_horaria.py`, `test_duplicar_cotizacion.py`, `TestTiempoAdicional`, `._login_admin`, `test_cotizacion_autorrelleno.py`, `test_preguntar_datos.py`, `test_aviso_referencia.py`, `payroll_detail.html`, `test_nav_movil.py`, `test_agenda_cajon.py`, `test_cotizacion_publica.py`, `._preguntar`, `TestEditarUnInstalador`, `TestFullCarAbsorbeLoExterior`, `test_parqueadero.py`, `_borrar`, `test_solicitud_precios.py`, `_cita`, `test_cotizaciones.py`, `._login`, `TestTraerLosPreciosAUnaCotizacion`, `test_colores_agenda.py`, `TestPanelManual`, `TestPromptExigeDosColumnas`?**
  _High betweenness centrality (0.392) - this node is a cross-community bridge._
- **Why does `login_as()` connect `make_user` to `_claude_responde`, `make_admin`, `test_abonos_ajustes.py`, `.admin`, `test_archivar_conversaciones.py`, `test_backfill_calificacion.py`, `TestVistaPreviaDelPrecio`, `test_saldos.py`, `test_lista_precios.py`, `test_filtro_fechas_whatsapp.py`, `test_festivos.py`, `test_servicios_ui.py`, `_conv`, `TestAgendaDeDiagnosticos`, `datetime`, `TestTiempoAdicional`, `test_preguntar_datos.py`, `test_agenda_cajon.py`, `._preguntar`, `test_parqueadero.py`, `_cita`, `test_colores_agenda.py`, `TestPanelManual`, `TestPromptExigeDosColumnas`?**
