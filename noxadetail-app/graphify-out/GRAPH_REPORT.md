...
## Corpus Check
- 40 files · ~167,814 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2227 nodes · 4207 edges · 115 communities (109 shown, 6 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 79 edges (avg confidence: 0.81)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `1693aedf`
...
Cohesion: 0.08
Nodes (22): payment_methods_toggle(), service_prices_toggle(), service_prices_update(), vehicle_types_toggle(), Sección 15: Catálogo (clasificación de vehículo Camioneta/SUV/Auto/Moto + servicios; cerámico ya incluye la corrección), Sección 10: Cierre en dos pasos (día, luego hora), confirmación final resumida, Sección 14: Qué es un coating cerámico (7 pasos, curado 12-18h), Sección 5: Formato de respuesta (300 caracteres, máx 3 mensajes, una pregunta por turno) (+14 more)

### Community 7 - "app.py"
Cohesion: 0.03
Nodes (56): _backfill_public_tokens(), ensure_adjustment_base_schema(), ensure_appointment_plan_schema(), ensure_outsourcing_duration_schema(), ensure_payroll_schema(), ensure_prioridad_sin_calificar(), ensure_quote_item_detail_schema(), ensure_quote_ppf_brands_schema() (+48 more)
...
Cohesion: 0.11
Nodes (19): bogota_now(), _filtro_dia_bogota(), _job_ceramic_3weeks(), _job_ceramic_followup(), _job_client_reminder(), _job_reengagement_followup(), _kpis_diagnosticos(), notify_admin_gestion_cliente() (+11 more)

### Community 18 - "plan_sell"
Cohesion: 0.07
Nodes (26): api_plan_price(), AppointmentOutsourcing, calculate_real_price(), _citas_sin_reclasificar(), es_marketing(), _format_planes_for_prompt(), _guardar_tercerizacion(), plan_sell() (+18 more)
...
Cohesion: 0.09
Nodes (23): abreviar_servicio(), abreviar_servicios(), _analytics_data(), api_events(), _diagnostic_service(), es_cita_de_diagnostico(), _job_post_service_followup(), _kpis_rentabilidad() (+15 more)
...
Cohesion: 0.10
Nodes (12): login_as(), TestApiDiaCerrado, Los saldos son información de la cuenta, no de la operación diaria., TestPaginaEstado, Quedan dos capas: el allowlist global OPERARIO_ENDPOINTS lo rebota con un 302…, TestAcceso, Borrarlo dejaría sin nombre la liquidación de las citas viejas., El desglose que se ve al agendar sale del servidor, con la misma fórmula que… (+4 more)

### Community 40 - "Appointment"
Cohesion: 0.07
Nodes (32): Agreement, agreements_create_alias(), agreements_quick_create(), api_estimate_price(), apply_adjustments(), apply_agreement_discount(), apply_agreement_discount_split(), Appointment (+24 more)
...
Cohesion: 0.13
Nodes (16): api_dia_cerrado(), book_diagnostic_from_bot(), _clean_phone_or_default(), es_festivo(), _find_active_appointment_by_plate(), motivo_dia_cerrado(), Devuelve el celular normalizado solo si parece un teléfono de verdad.…, Crea la cita de diagnóstico que Mariana cerró con el cliente. Nunca confía en… (+8 more)
...
Cohesion: 0.13
Nodes (15): agrupar_servicios(), api_preguntar(), categoria_de_servicio(), delete_service(), preguntar_view(), puede_borrar_servicios(), puede_preguntar_a_los_datos(), ok' | 'warn' | 'bad' según los umbrales del negocio. Devuelve cadena vacía si… (+7 more)
...
### Community 58 - "limit"
Cohesion: 0.09
Nodes (21): api_client_names(), api_client_plates(), api_notifications(), change_password(), _guardar_version_cliente(), _is_safe_redirect_target(), login(), quote_public() (+13 more)

### Community 59 - "Service"
Cohesion: 0.15
Nodes (10): color_hex_valido(), color_texto_legible(), Normaliza un color a #RRGGBB, o None si no lo es. El valor viaja desde un…, Negro o blanco, el que contraste con el fondo. Es el valor por defecto cuando…, Crea servicios base si la tabla está vacía., Color del cajón de la cita en la agenda. Se valida el hex acá y no solo en el…, seed_new_services(), seed_services() (+2 more)

### Community 60 - "Expenses List (DataTable)"
Cohesion: 0.09
Nodes (25): Expense, expense_categories_delete(), expense_categories_list(), expense_categories_new(), expense_categories_toggle(), ExpenseCategory, expenses_edit(), expenses_list() (+17 more)
...
### Community 62 - "_int_o_cero"
Cohesion: 0.07
Nodes (30): _catalogo_para_cotizar(), _catalogo_ppf(), _int_o_cero(), _leer_formulario_de_cotizacion(), _nuevo_codigo_cotizacion(), ppf_marcas_activas(), ppf_prices_list(), PpfPrice (+22 more)

### Community 63 - "edit_appointment"
Cohesion: 0.08
Nodes (30): api_client_by_plate(), api_plans_by_plate(), AppointmentOperator, calculate_real_duration_minutes(), Client, edit_appointment(), _minutos_extra_tercerizacion(), new_appointment() (+22 more)
...
Cohesion: 0.15
Nodes (14): analytics_dashboard(), _kpis_clientes(), Recurrencia: en detailing conseguir un cliente cuesta mucho más que hacerlo…, Analytics Dashboard, Detail Drill-down Modal (click chart bar/point), Revenue Chart with Selectable Granularity (day/week/month/quarter/year), Sticky KPI Strip, Money Formatting Macro (data-v attribute) (+6 more)
...
Cohesion: 0.05
Nodes (56): api_public_web_lead(), _build_web_lead_opening_text(), _generate_and_send_reply(), _guardar_media_entrante(), _job_admin_reminder(), _log_outbound(), _looks_like_welcome_menu(), Message (+48 more)
...
### Community 89 - "seguimiento_gestionar"
Cohesion: 0.40
...
### Community 94 - "ensure_whatsapp_canal_schema"
Cohesion: 0.50
...
### Community 113 - "date"
Cohesion: 0.12
Nodes (15): _domingo_de_pascua(), festivos_colombia(), _format_festivos_for_prompt(), _liquidacion_instaladores(), liquidacion_instaladores_view(), Festivos que caen dentro de la ventana de agendamiento. El bloque de…, Cuánto se le debe a cada instalador en el periodo, trabajo por trabajo., Cuánto se le debe a cada instalador por el periodo, trabajo por trabajo. Sale… (+7 more)
...
### Community 125 - "_filtro_hace_cuanto"
Cohesion: 0.29
...
- **Why does `make_user()` connect `make_user` to `make_admin`, `test_abonos_ajustes.py`, `test_archivar_conversaciones.py`, `test_backfill_calificacion.py`, `Conversation`, `TestEliminar`, `_cita`, `test_colores_agenda.py`, `TestPanelManual`, `test_saldos.py`, `login_as`, `test_servicios_ui.py`, `_conv`, `_login_admin`, `._login_admin`, `datetime`, `TestTiempoAdicional`, `TestFormulario`, `test_preguntar_datos.py`, `test_parqueadero.py`, `TestAgendaDeDiagnosticos`, `test_nav_movil.py`, `TestRegresionProduccion`, `._preguntar`, `TestPromptExigeDosColumnas`, `test_festivos.py`, `_borrar`, `_claude_responde`, `test_cotizaciones.py`, `._login`, `.test_sin_porcentaje_valido_cae_al_del_catalogo`, `_borrar`, `test_cotizacion_publica.py`, `test_lista_precios.py`?**
  _High betweenness centrality (0.323) - this node is a cross-community bridge._
- **Why does `login_as()` connect `login_as` to `make_user`, `make_admin`, `test_abonos_ajustes.py`, `test_archivar_conversaciones.py`, `test_backfill_calificacion.py`, `_cita`, `test_colores_agenda.py`, `TestPanelManual`, `test_saldos.py`, `test_servicios_ui.py`, `_conv`, `datetime`, `TestTiempoAdicional`, `TestFormulario`, `test_preguntar_datos.py`, `test_parqueadero.py`, `TestAgendaDeDiagnosticos`, `TestRegresionProduccion`, `._preguntar`, `TestPromptExigeDosColumnas`, `test_festivos.py`, `_claude_responde`, `.test_sin_porcentaje_valido_cae_al_del_catalogo`, `test_lista_precios.py`?**
