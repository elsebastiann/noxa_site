...
## Summary
- 1050 nodes · 2130 edges · 67 communities (59 shown, 8 thin omitted)
...
Cohesion: 0.17
Nodes (8): login_as(), El formulario manda listas paralelas; acá se prueba el parseo., El default acordado con la operación: si nadie elige, es sobre lista., TestFormulario, Dos agendas con la misma pantalla: la que factura y la de diagnósticos., Si el cliente aprovechó y agendó también un servicio, ya factura., Todos los cajones dirían lo mismo; el renglón rinde más con las notas., TestAgendaDeDiagnosticos
...
### Community 7 - "app.py"
Cohesion: 0.06
Nodes (21): ensure_adjustment_base_schema(), ensure_appointment_plan_schema(), ensure_payroll_schema(), ensure_service_sales_schema(), _fetch_twilio_media_base64(), inject_user(), payment_methods_new(), PaymentMethod (+13 more)
...
Cohesion: 0.17
Nodes (15): api_public_mb_availability(), api_public_mb_available_days(), api_public_mb_book(), _appointment_capacity_profile(), _day_business_end(), get_available_days(), get_available_slots(), Para una cita existente, determina (es_solo_diagnostico, fin_ocupacion_cupo).… (+7 more)
...
Cohesion: 0.50
Nodes (4): api_dia_cerrado(), motivo_dia_cerrado(), Por qué está cerrado ese día, en texto para el cliente. None si se atiende., ¿Se atiende ese día? Lo consulta el formulario de citas para avisar antes de…
...
Cohesion: 0.16
Nodes (11): payroll_delete(), payroll_detail(), payroll_entry_update(), payroll_list(), payroll_pay(), payroll_vale_new(), Vale de adelanto de un operario., Vale (+3 more)
...
### Community 29 - "_diagnostic_availability"
Cohesion: 0.25
Nodes (8): _availability_vehicle_type_id(), _diagnostic_availability(), es_dia_habil(), es_festivo(), Nombre del festivo si esa fecha lo es, o None., True si NOXA atiende ese día: día hábil de la semana y no festivo., El diagnóstico dura lo mismo para cualquier vehículo, así que para calcular…, [(fecha, [horas libres]), ...] de los próximos días hábiles con cupo.
...
Cohesion: 0.16
Nodes (13): bogota_now(), book_diagnostic_from_bot(), _find_active_appointment_by_plate(), _job_client_reminder(), plans_list(), Activa y dentro de fechas. Las fechas vacías significan "sin límite"., Ahora' en hora de Bogotá, naive — que es como se guardan start_datetime /…, Crea la cita de diagnóstico que Mariana cerró con el cliente. Nunca confía en… (+5 more)
...
Cohesion: 0.17
Nodes (12): api_estimate_price(), api_public_mb_price(), apply_adjustments(), appointment_already_closed(), calculate_real_price(), close_appointment(), Calcula el precio base real usando ServicePrice. Estrategia: - Suma los precios…, Busca en producción el Agreement activo que corresponde al tier del socio. (+4 more)

### Community 49 - "_transacciones_citas"
Cohesion: 0.22
...
Cohesion: 0.13
Nodes (16): abreviar_servicio(), abreviar_servicios(), api_events(), appointment_json(), appointment_money(), calculate_estimated_amount_for_appointment(), es_cita_de_diagnostico(), es_operario() (+8 more)
...
- **Why does `make_user()` connect `make_user` to `login_as`, `TestPanelManual`, `make_admin`, `test_abonos_ajustes.py`, `test_parqueadero.py`, `test_backfill_calificacion.py`, `User`, `datetime`, `conftest.py`?**
...
- **Why does `login_as()` connect `login_as` to `make_user`, `TestPanelManual`, `test_abonos_ajustes.py`, `test_parqueadero.py`, `make_admin`, `test_backfill_calificacion.py`, `datetime`, `conftest.py`?**
