...
## Corpus Check
- 29 files · ~132,634 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1566 nodes · 3021 edges · 82 communities (76 shown, 6 thin omitted)
- Extraction: 97% EXTRACTED · 3% INFERRED · 0% AMBIGUOUS · INFERRED: 77 edges (avg confidence: 0.81)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `254fb1b9`
...
### Community 10 - "route"
Cohesion: 0.05
Nodes (57): agreements_create_alias(), agreements_quick_create(), api_client_by_name(), api_public_stats_appointments_count(), calendar_diagnosticos(), expense_categories_rename(), index(), installer_toggle() (+49 more)
...
Cohesion: 0.21
Nodes (12): _clean_phone_or_default(), _historial_ceramico(), _normalize_whatsapp_number(), Quién ya tiene una cita por delante. Es la confirmación objetiva de que la…, {telefono: (fecha_ultima_visita, servicios, monto)} de citas completadas., {telefono: fecha del último cerámico o de su último mantenimiento}. Se mira el…, Arma el tablero completo. Cada persona cae en UNA sola columna., Normaliza un número al formato E.164 que usa Twilio/WhatsApp (+57 por defecto,… (+4 more)
...
Cohesion: 0.09
Nodes (18): color_hex_valido(), color_texto_legible(), Crea servicios base si la tabla está vacía., Normaliza un color a #RRGGBB, o None si no lo es. El valor viaja desde un…, Negro o blanco, el que contraste con el fondo. Es el valor por defecto cuando…, Crea o actualiza el precio de una celda de la matriz. Hace falta aparte de…, Gestión simple de servicios: ver y agregar nuevos., Color del cajón de la cita en la agenda. Se valida el hex acá y no solo en el… (+10 more)
...
### Community 20 - "_call_claude"
Cohesion: 0.11
Nodes (20): _build_message_history(), _call_claude(), _diagnostico_anthropic(), _diagnostico_de(), _fecha_hoy_para_prompt(), generate_followup_message(), _get_claude_client(), _linea_perfil() (+12 more)
...
### Community 23 - "User"
Cohesion: 0.12
Nodes (14): api_client_names(), api_client_plates(), change_password(), _is_safe_redirect_target(), login(), True si el empleado aún está en período de prueba (primer mes desde hire_date)., Evita "open redirect": el 'next' debe ser una ruta propia (/algo), nunca una…, seed_demo_data() (+6 more)
...
Cohesion: 0.06
Nodes (38): api_public_mb_availability(), api_public_mb_book(), api_public_mb_price(), notify_admin_mercedes_benz_booking(), payment_methods_toggle(), Busca en producción el Agreement activo que corresponde al tier del socio., Avisa por WhatsApp al admin cuando un socio del club Mercedes-Benz se…, Devuelve (services, error). Solo servicios activos y marcados… (+30 more)
...
Cohesion: 0.08
Nodes (27): Agreement, agreements_list(), agreements_new(), agreements_toggle(), api_estimate_price(), apply_adjustments(), apply_agreement_discount(), apply_agreement_discount_split() (+19 more)
...
### Community 41 - "make_user"
Cohesion: 0.11
Nodes (9): make_user(), TestInTrial, Los saldos son información de la cuenta, no de la operación diaria., TestPaginaEstado, Quedan dos capas: el allowlist global OPERARIO_ENDPOINTS lo rebota con un 302…, TestAcceso, TestPantalla, Borrarlo dejaría sin nombre la liquidación de las citas viejas. (+1 more)

### Community 42 - "send_whatsapp"
Cohesion: 0.22
Nodes (10): notify_admin_escalation(), _public_base_url(), Devuelve (numero_sin_prefijo, error). El sender de producción de NOXA es el…, Dominio público de la app, para que Twilio sepa a dónde devolver los callbacks…, Envía un mensaje de WhatsApp via Twilio. OJO con el valor de retorno: `ok=True`…, Avisa al admin por WhatsApp cuando Mariana detecta una señal de negocio que…, send_whatsapp(), _status_callback_url() (+2 more)
...
Cohesion: 0.11
Nodes (20): _clasificar_conversacion_historica(), _compute_priority(), _generate_and_send_reply(), _looks_like_welcome_menu(), _match_valor_cerrado(), notify_admin_bot_booking(), notify_admin_bot_reschedule(), _parse_agendar_marker() (+12 more)
...
Cohesion: 0.20
Nodes (9): _gestiones_activas(), _puede_ver_seguimiento(), Lo que un humano hizo con una tarjeta del tablero de seguimiento. Existe porque…, El tablero de pipeline: leads y clientes que necesitan que alguien los contacte…, Marca una tarjeta como contactada, pospuesta o descartada. Se hace upsert sobre…, Devuelve (ocultas, escritas). Están separadas porque escribirle a alguien NO…, seguimiento_gestionar(), seguimiento_tablero() (+1 more)
...
Cohesion: 0.07
Nodes (29): abreviar_servicio(), abreviar_servicios(), analytics_detalle(), api_events(), appointment_json(), calendar_view(), _diagnostic_service(), es_cita_de_diagnostico() (+21 more)
...
### Community 67 - "puede_ver_finanzas"
Cohesion: 0.22
Nodes (9): es_marketing(), liquidacion_instaladores_view(), plan_toggle(), plans_list(), puede_ver_finanzas(), Planes vendidos, con su saldo. Lo primero que se necesita saber es a quién le…, Desactiva un plan vendido (venta anulada, cliente que se fue)., Cuánto se le debe a cada instalador en el periodo, trabajo por trabajo. (+1 more)
...
Cohesion: 0.06
Nodes (38): agrupar_servicios(), analytics_dashboard(), _analytics_data(), categoria_de_servicio(), _kpis_clientes(), _kpis_diagnosticos(), _kpis_embudo(), _kpis_operacion() (+30 more)
...
### Community 73 - "login_as"
Cohesion: 0.10
Nodes (14): datetime, _clean_db(), client(), login_as(), fixture, Cada test arranca con las tablas de nómina/usuarios vacías, y corre dentro de…, _session_setup(), Lo que va dentro del cajón de una cita en la agenda. El recorte por alto lo… (+6 more)
...
### Community 80 - "_tomar_snapshot_costo_railway"
Cohesion: 0.50
...
- **Why does `make_user()` connect `make_user` to `TestFormulario`, `test_colores_agenda.py`, `make_admin`, `test_abonos_ajustes.py`, `TestPanelManual`, `test_parqueadero.py`, `test_lista_precios.py`, `test_archivar_conversaciones.py`, `login_as`, `test_saldos.py`, `test_backfill_calificacion.py`, `_conv`, `TestTiempoAdicional`, `.test_sin_porcentaje_valido_cae_al_del_catalogo`, `User`, `TestVistaPreviaDelPrecio`, `_cita`, `TestAgendaDeDiagnosticos`?**
  _High betweenness centrality (0.178) - this node is a cross-community bridge._
- **Why does `login_as()` connect `login_as` to `TestFormulario`, `test_colores_agenda.py`, `make_admin`, `test_abonos_ajustes.py`, `TestPanelManual`, `test_parqueadero.py`, `test_lista_precios.py`, `test_archivar_conversaciones.py`, `test_saldos.py`, `make_user`, `test_backfill_calificacion.py`, `_conv`, `TestTiempoAdicional`, `.test_sin_porcentaje_valido_cae_al_del_catalogo`, `TestVistaPreviaDelPrecio`, `_cita`, `TestAgendaDeDiagnosticos`?**
