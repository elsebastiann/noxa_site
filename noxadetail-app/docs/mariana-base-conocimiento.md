...
2. **SEGUIMIENTO A LEADS EN SILENCIO** — 3 etapas: `recuperar_intencion` (24h), `reabrir_conversacion` (72h), `cierre_elegante` (7d, último intento). Prohibido lo genérico ("¿sigues ahí?", "quedo atento"). Máx ~300 caracteres. **En `ancla_de_valor`, si el lead se enfrió por precio, hay dos ángulos y elige uno: perspectiva de valor (costo por año/día), o puerta de entrada más barata — corrección de pintura sola, sin sellado (2026-08-24).** Esa alternativa se presenta como OTRO servicio, nunca como descuento del cerámico ya cotizado, y su precio sale del bloque de PRECIOS VIGENTES; si no está ahí, se ofrece sin cifra y se pasa al diagnóstico. Ojo: si `Corrección de pintura` no tiene precio cargado en `service_prices`, Mariana no va a poder dar un valor — se arregla en el panel, no en el prompt.
...
6. **MEDIOS DE PAGO** — Efectivo, transferencia, datáfono. Anticipo 10%: Bre-B `1024501327`, Daviplata `3143068701`, Nequi `3143068701`. Esto lo maneja ella sola, sin escalar. **Intensidad del anticipo (2026-08-24): es una respuesta, no un gancho.** No lo saca ella por su cuenta, no lo repite si el cliente no lo tomó, y nunca lo pone como condición para agendar o para el diagnóstico (que es gratis). El prompt nunca le pidió empujarlo — lo hacía porque el tema estaba suelto, así que la regla es una restricción explícita.
7. **HORARIO** — Lunes a sábado, 9:00–18:00. Nunca domingo **ni festivo colombiano** (2026-08-15). Los festivos se calculan con `festivos_colombia(año)` (Ley Emiliani + fechas móviles de Pascua), no se listan a mano; el filtro compartido es `es_dia_habil(fecha)` y el motivo legible para el cliente lo da `motivo_dia_cerrado(fecha)`. El bloqueo real vive dentro de `get_available_slots()`, así que lo heredan el widget del club, el panel y el bot. Además se le inyecta a Mariana en cada turno la lista de festivos que caen en la ventana de agendamiento (`_format_festivos_for_prompt`) para que sepa nombrarlos si el cliente propone uno.
...
16b. **REAGENDAMIENTO** — un cliente que ya tenía cita y escribe para moverla no es un lead del embudo: se marca `Reagendado`, no `Diagnóstico agendado` (2026-08-16). El estado lo fija el código tras un `[REAGENDAR:]` exitoso, así que gana sobre el `[META:]` del modelo. Va en `ESTADOS_CON_CITA` junto a los otros dos estados con cita en firme — esa tupla es la que usa el job de seguimiento para no perseguir a quien ya agendó, y las analíticas para contar leads con cita; olvidarlo ahí revive el incidente del 2026-08-10.
...
18. **MARCADORES INTERNOS** — `[META: estado=...; servicios=...]` en **cada** turno (estados: En proceso / Diagnóstico agendado / Servicio agendado; tags: Cerámico / PPF o wrap / Otro servicio) y `[NOMBRE: ...]` cuando el cliente da su nombre real. **`[ESPERAR: AAAA-MM-DD]` cuando se acuerda volver a hablar más adelante (2026-08-28)**: guarda `conversation.seguimiento_pausado_hasta` y `_candidatas_de_seguimiento()` excluye la conversación hasta esa fecha. Salió de un caso real: el cliente dijo "esta semana no puedo", quedaron en hablar la próxima semana, y el job le escribió AL DÍA SIGUIENTE — el job decide por tiempo transcurrido y no tenía forma de enterarse del acuerdo. La fecha la calcula Mariana con el bloque de FECHA Y HORA ACTUAL que ya recibe cada turno.
