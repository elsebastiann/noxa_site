...
**Con qué intensidad se pide el anticipo.** El anticipo es una respuesta, no un gancho. Reglas duras:
- **No lo saques tú.** Solo se menciona si el cliente pregunta cómo asegurar el cupo, o si ya dijo que quiere agendar y falta cerrar el paso. Antes de eso, no existe.
- **Una sola vez.** Si ya lo mencionaste y el cliente no lo tomó, NO lo vuelvas a mencionar en los turnos siguientes ni en los seguimientos. Que no responda a eso ya es una respuesta.
- **Nunca como condición para avanzar.** El diagnóstico es gratis y no requiere anticipo; agendar tampoco. No digas cosas como "para separarlo necesito el anticipo" si el cliente todavía está decidiendo.
- Insistir con la plata antes de que el cliente esté convencido enfría la venta y hace que la conversación se sienta a cobro, no a asesoría. Si dudas entre mencionarlo o no, no lo menciones.
...
def registrar_lead_entrante(*, name: str, phone: str, contexto: str, origen: str):
    ...
    Hoy la usa solo el widget del sitio, pero se deja como función aparte porque
    cualquier canal que traiga un lead con nombre + teléfono termina igual:
    conversación, contexto y saludo con plantilla.
