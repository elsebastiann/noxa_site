...
This is now a single git repo (unified 2026-08-08 via `git subtree`, full commit history of both preserved). `noxadetail/` is the public marketing site, **live on Railway** (project `noxa-site`, sirviendo `www.noxadetail.com`, `rootDirectory: noxadetail`, builder Railpack → Caddy estático). Un `git push` lo publica solo, igual que la app — verificado el 2026-08-19. Antes estaba en Hostinger y este archivo decía "not yet live" en Railway; ya no aplica, y creerlo lleva a subir archivos a mano sin necesidad.

Ojo: Railway reconstruye **los dos** servicios ante cualquier push al repo, aunque el cambio sea de un solo subproyecto. Un commit que solo toca `noxadetail-app/` igual dispara un build del sitio, así que un fallo de build ahí no significa que se haya roto el sitio. `noxadetail-app/` is the business-management app (app.noxadetail.com, Flask, Railway project `ERP`/service `web`) — it has its own `graphify-out/` and its own `noxadetail-app/CLAUDE.md`. It's where most day-to-day code changes happen (agreements/tiers, payroll, Mariana WhatsApp bot, appointments).
